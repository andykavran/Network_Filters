function network_metadata = network_community_filter(metadata, edge_list, labels, filter, varargin)
% NETWORK_COMMUNITY_FILTER Modifies node metadata using smoothen, sharpen, or median filters.
...
for ii=1:num_nodes
    ...
    try
        neigh_community = labels(neighbors(1:count)); 
    catch
        error('We have a problem with ii = %d', ii);
    end
    switch(filter)
        case {'mean', 'Mean', 'smooth','Smooth','smoothen', 'Smoothen'}
            %average all neighbors and self in same community
            use_these = neigh_community==labels(ii);
            network_metadata(ii) = mean([neigh_metadata(use_these)', self_meta]);
...
%             network_metadata(ii) = self_meta + multiplier .* (self_meta- local_mean);
        case {'median', 'Median'}
            %median of all neighbors and self in same community
            use_these = neigh_community==labels(ii);
            network_metadata(ii) = median([neigh_metadata(use_these)', self_meta]);
        otherwise
